...
static void bindit (int sock, char *name)
{
  ...
  if (bind(sock, (struct sockaddr *)&addr, sizeof(sa_family_t)) == -1) /* autobind */
    strerr_diefusys(111, "bind") ;
  if (getsockname(sock, (struct sockaddr *)&addr, &addrlen) == -1)
    strerr_diefusys(111, "getsockname") ;
  if (addr.sun_path[0])
    strerr_diefu(111, "autobind to an abstract socket") ;
...
static inline int run_child (int sock, int fd, unsigned int timeout)
{
  ...
  for (;;)
  {
    ...
    if (!r) strerr_dief(99, "timed out waiting for notification") ;
    r = sanitize_read(fd_recv(sock, buf, 8191, 0)) ;
    if (r == -1)
    {
      if (errno == EPIPE) _exit(0) ;
      else strerr_diefusys(111, "recv") ;
    }
    if (r)
    {
      buf[r++] = 0 ;
...
int main (int argc, char const *const *argv)
{
  ...
  else
  {
    ...
    if (!r) strerr_dief(100, "invalid ", "./notification-fd") ;
  }
  if (fcntl(fd, F_GETFD) == -1)
    strerr_diefsys(111, "notification-fd", " sanity check failed") ;

  sock = ipc_datagram_nbcoe() ;
  if (sock == -1) strerr_diefusys(111, "create socket") ;
  bindit(sock, modif + 15) ;

  pid = wgolb & GOLB_SINGLEFORK ? fork() : doublefork() ;
  if (pid == -1) strerr_diefusys(111, wgolb & GOLB_SINGLEFORK ? "fork" : "doublefork") ;
